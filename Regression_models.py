...
seebeck = client.get_data(
{
    "classes": "binary",
    "props": "seebeck coefficient"
})
seebeck = pd.DataFrame(seebeck, columns=['Phase', 'Formula', 'SG', 'Entry', 'Property', 'Units', 'Value'])
...
structure = pd.DataFrame(structure, columns=['Phase', 'Entry', 'Formula','cell_abc', 'SG',  'Basis_noneq', 'els_noneq'])
...
model_linear = tc.linear_regression.create(total,target = 'Value', features =['APF', 'Wiener'])
...
model_decision = tc.decision_tree_regression.create(train_d,target = 'Value', features = ['APF', 'Wiener'])
...
model_boosted = tc.boosted_trees_regression.create(train_b,target = 'Value', features = ['APF', 'Wiener'])
...
model_random = tc.random_forest_regression.create(train_r,target = 'Value', features = ['APF', 'Wiener'])
