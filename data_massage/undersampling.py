...
# leave only no more than 5 structures for each Seebeck
for row in sorted(data):
    if row[0] not in seeb_used:
        cnt = 1
        seeb_used.append(row[0])
        new_list_seeb.append(row[0])
...
df_seebeck.to_csv('/root/projects/ml-playground/data_massage/seebeck_coefficient_and_structure/data/01_04/under_seeb_200_3.csv', index=False)
df_structs.to_csv('/root/projects/ml-playground/data_massage/seebeck_coefficient_and_structure/data/01_04/under_str_200_3.csv', index=False)
